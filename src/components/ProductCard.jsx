...
export default function ProductCard({ cate, as: CardTag = "li", className = "", onClick }) {
  const { items, onColorCode, onAddCart, openCart, wishList, onAddWishList, onRemoveWish, onLoadWishList } = useProductStore();
  ...
  const selectedSoldOut = selectedSizeIndex !== -1 && Boolean(selectedVariant.soldout?.[selectedSizeIndex]);

  const isSoldOut = selectedSoldOut || (
  ...
  const badgeItems = isSoldOut
    ...
    : [
      ...(cate.tag || []).map((tag) => ({ key: tag, label: tag, type: "default" })),
      ...(cate.discountRate > 0 ? [{ key: `discount-${cate.discountRate}`, label: "SALE", type: "discount" }] : []),
    ];
  ...
  const getColorStyle = (colorName) => {
    ...
    return colorValue.includes("gradient") ? { background: colorValue } : { backgroundColor: colorValue };
  };
  ...
  const handleAddCart = (event) => {
    ...
    const cartPrice = selectedVariant.discountRate > 0
  ...
  const handleBuyNow = (event) => {
    ...
    const salePrice = selectedVariant.discountRate > 0
      ? selectedVariant.discountPrice
      : selectedVariant.price;
    ...
    if (!user) {
      openLogin(orderItems); // 비로그인 시 로그인 모달 + 비회원주문 버튼
      return;
    }
  ...
  return (
    ...
    >
            ...
            <span
              ...
              onClick={(event) => {
                ...
                const nextSizeIndex = Array.isArray(variant.sizes)
                  ? variant.sizes.findIndex((size, index) => size && !variant.soldout?.[index])
                  : -1;
                setSelectedSize(nextSizeIndex >= 0 ? variant.sizes[nextSizeIndex] : variant.sizes?.[0] || "");
              }}
            />
          );
        })}
        ...
        <div className="card-size-list" aria-label={`${cate.name} 사이즈 목록`}>
          {selectedVariant.sizes.filter(Boolean).map((size, index) => (
            <span
              ...
              className={`${selectedVariant.soldout?.[index] ? "is-soldout" : ""} ${selectedSize === size ? "selected" : ""}`}
              onClick={(event) => {
                event.stopPropagation();
                if (!selectedVariant.soldout?.[index]) setSelectedSize(size);
              }}
            >
              {size}
            </span>
      ...
      >
        ...
        <button type="button" className="card-action-btn" onClick={handleAddCart} disabled={isSoldOut}>
          장바구니
        </button>

        <button type="button" className="card-action-btn" onClick={handleBuyNow} disabled={isSoldOut}>
          구매하기
        </button>
      </div>
    </CardTag>
